...
impl Writer {
    ...
    pub fn draw_centered_block(&mut self, lines: &[&str]) {
        self.clear_screen();

        let start_row = (BUFFER_HEIGHT.saturating_sub(lines.len())) / 2;

        for (index, line) in lines.iter().enumerate() {
            let row = start_row + index;
            let line_width = line.len().min(BUFFER_WIDTH);
            let start_col = (BUFFER_WIDTH - line_width) / 2;

            for (offset, byte) in line.bytes().take(BUFFER_WIDTH).enumerate() {
    ...
    fn update_cursor(&self) {
        let cursor_position = ((BUFFER_HEIGHT - 1) * BUFFER_WIDTH + self.column_position) as u16;
